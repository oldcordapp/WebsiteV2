...
const Header = ({ onGetStarted, textColorClass }) => (
  <div className={styles.section}>
    <div className={styles.inner}>
      <Link to="/" className={`${styles["logo-link"]} ${styles[textColorClass]}`}>
        <Logo className={styles.logo} />
      </Link>
      
      <ul className={`${styles.navigation} ${textColorClass} ${styles[textColorClass]}`}>
        {NAV_ITEMS.map(({ to, href, label }) => (
          <li key={label} className={`${styles["list-item"]} ${styles["list-item-push-left"]}`}>
      ...
      <ul className={`${styles["navigation-right"]} ${textColorClass} ${styles[textColorClass]}`}>
        {SOCIAL_LINKS.map(({ href, Icon }) => (
          <li key={href} className={`${styles["list-item"]} ${styles["list-item-push-right"]}`}>
            <a href={href}><Icon className={styles["list-item-socials"]}/></a>
          </li>
        ))}
